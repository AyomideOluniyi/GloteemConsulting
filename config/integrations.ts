/**
 * Gloteem Consulting - Integration Configuration
 * All external URLs and contact details are centralized here.
 */

export const INTEGRATIONS = {
  calendly: {
    url: 'https://calendly.com/enquiry-gloteemconsulting',
    embedType: 'inline' as const,
  },
  stripe: {
    paymentLink: 'https://buy.stripe.com/PLACEHOLDER',
  },
  amazon: {
    bookUrl: 'https://www.amazon.co.uk/gp/aw/d/B0GK2LWNS1',
  },
  email: {
    contact: 'glory@gloteemconsulting.com',
    personal: 'gloryadebanjo21@gmail.com',
  },
  social: {
    linkedin: 'https://www.linkedin.com/in/gloryat/',
    youtube: 'https://www.youtube.com/@gloryadebanjo517',
    facebook: 'https://www.facebook.com/share/17ymTbwLG8/',
    instagram: 'https://www.instagram.com/glory.adebanjo',
  },
  domain: 'gloteemconsulting.com',
};

export const BRAND = {
  name: 'Gloteem Consulting',
  founder: 'Glory Adebanjo-Taiwo',
  tagline: 'Gloteem Consulting provides leadership development, organisational strategy and healthcare consultancy services for growing businesses and care providers.',
  shortTagline: 'Leadership development, organisational strategy and healthcare consultancy.',
};
